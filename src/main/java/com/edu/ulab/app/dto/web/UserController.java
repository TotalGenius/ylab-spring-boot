...
import com.edu.ulab.app.exception.ValidException;
...
public class UserController {
    ...
    public UserBookResponse createUserWithBooks(@RequestBody @Valid UserBookRequest request, BindingResult bindingResult,
                                                @RequestHeader(WebConstant.RQID) @Pattern(regexp = WebConstant.REQUEST_ID_PATTERN) final String requestId) {
        if (bindingResult.hasErrors()) throw new ValidException(bindingResult.toString());
    ...
    @Operation(summary = "Update user and books row.",
            responses = {
                    @ApiResponse(description = "Person book",
                            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                                    schema = @Schema(implementation = UserBookResponse.class)))})
    public UserBookResponse updateUserWithBooks(@RequestBody @Valid UserBookRequest request, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) throw new ValidException(bindingResult.toString());
